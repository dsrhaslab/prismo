...
namespace stream{
    ...
    struct Resources{
        ...
        size_t readFrom(std::istream& in){
            in.read((char*)buffIn.data(), static_cast<std::streamsize>(buffInSize));
            return static_cast<size_t>(in.gcount());
        }

        void writeTo(std::ostream& out, size_t pos = 0){
            out.write((char*)buffOut.data(), static_cast<std::streamsize>(pos));
        }
    ...
    struct Context{
        ...
        Context([[maybe_unused]] compress_level_t _compress_level, threads_number_t nThreads)
    ...
    inline void decompress(
        std::istream& in,
        std::ostream& out,
        [[maybe_unused]] threads_number_t nThreads = 1
    ){
        Resources res{};
        Context ctx{};

        [[maybe_unused]] size_t const toRead = res.getToRead();
        size_t read;
        size_t lastRet = 0;
        int isEmpty = 0;
...
inline buffer_t compress( buffer_t const& data, [[maybe_unused]] compress_level_t compress_level = 3 ){
    buffer_t comp_buffer{};
    [[maybe_unused]] size_t est_compress_size = inplace::compress(data, comp_buffer);
    return comp_buffer;
}

inline buffer_t decompress(buffer_t &data) {
  buffer_t decomp_buffer{};
  [[maybe_unused]] auto const est_decomp_size = inplace::decompress(data, decomp_buffer);
