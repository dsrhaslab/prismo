       ...
       public:
        ...
        explicit TraceReader(const json& j) {
            ...
            if (memory_bytes < sizeof(Trace::Record)) {
                throw std::invalid_argument(
                    "Memory should be equal or greater than " +
        ...
        std::optional<Trace::Record> next_record(void) {
            if (current_position >= records_read) {
                ...
                trace_file.read(reinterpret_cast<char*>(buffer.data()),
                                bytes_to_read);
                const std::streamsize bytes_read = trace_file.gcount();

                // std::cout << "Reading " << bytes_to_read << " bytes from trace file" <<
                // std::endl; std::cout << bytes_read << " bytes read from trace file" <<
                // std::endl;

                records_read = static_cast<std::size_t>(bytes_read) /
                               sizeof(Trace::Record);
                current_position = 0;
