...
namespace Engine {
    ...
    class SpdkEngine : public Engine {
       private:
        ...
        static int start_spdk_app(void* spdk_engine, const SpdkConfig config,
                                  std::atomic<TriggerData>* trigger_atomic);

        static void bdev_event_cb(enum spdk_bdev_event_type type,
                                  struct spdk_bdev* bdev, void* event_ctx);

        static void io_complete(struct spdk_bdev_io* bdev_io, bool success,
                                void* cb_arg);
        ...
        static void init_threads(std::vector<uint32_t> pinned_cores,
        ...
        static void* allocate_dma_buffer(SpdkAppContext* app_context,
        ...
        static void thread_main_dispatch(
            ...
            std::vector<SpdkThreadCallBackContext*>& thread_cb_contexts,
            moodycamel::ConcurrentQueue<int>& available_indexes,
            uint8_t* dma_buf, int block_size);
       ...
       public:
        explicit SpdkEngine(std::unique_ptr<Metric::Metric> _metric,
