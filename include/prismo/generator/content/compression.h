...
namespace Generator {
    ...
    inline void apply_compression(uint8_t* buffer, size_t size,
                                  uint32_t compression) {
        size_t compressed_size =
            (size - sizeof(BlockMetadata::block_id)) * compression / 100;
        std::memset(buffer + sizeof(BlockMetadata::block_id), 0,
                    compressed_size);
    }
