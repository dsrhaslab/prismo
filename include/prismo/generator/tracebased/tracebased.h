...
namespace Generator {
    ...
    class TraceBasedAccessGenerator : public AccessGenerator,
                                      public TraceBased {
       public:
        ...
        explicit TraceBasedAccessGenerator(
            const json& j, std::unique_ptr<Extension::TraceExtension> ext)
            : AccessGenerator(j), TraceBased(std::move(ext)) {}

        void validate(void) const override { AccessGenerator::validate(); }
    ...
    class TraceBasedOperationGenerator : public OperationGenerator,
                                         public TraceBased {
       public:
        TraceBasedOperationGenerator() = delete;

        virtual ~TraceBasedOperationGenerator() {
            std::cout << "~Destroying TraceBasedOperationGenerator"
                      << std::endl;
        }

        explicit TraceBasedOperationGenerator(
    ...
    class TraceBasedContentGenerator : public ContentGenerator,
                                       public TraceBased {
       public:
        ...
        explicit TraceBasedContentGenerator(
            const json& j, std::unique_ptr<Extension::TraceExtension> ext)
        ...
        BlockMetadata next_block(uint8_t* buffer, size_t size) override {
            ...
            return BlockMetadata{.block_id = record.block_id, .compression = 0};
        }
    };
