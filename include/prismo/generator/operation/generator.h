...
namespace Generator {
    ...
    class PercentageOperationGenerator : public OperationGenerator {
       private:
        ...
        std::vector<PercentageElement<uint32_t, Operation::OperationType>>
            op_percentages;
       ...
       public:
        ...
        ~PercentageOperationGenerator() override {
            std::cout << "~Destroying PercentageOperationGenerator"
                      << std::endl;
        };
        ...
        explicit PercentageOperationGenerator(const json& j) : rng(0, 99) {
            ...
            for (const auto& item : j.at("percentages").items()) {
                ...
                op_percentages.push_back(
                    PercentageElement<uint32_t, Operation::OperationType>{
        ...
        Operation::OperationType next_operation(void) override {
            return select_from_percentage_vector(rng.nextValue(),
                                                 op_percentages);
        };
        ...
        void validate(void) const override {
            if (operations.size() == 0) {
                throw std::invalid_argument(
                    "validate: invalid sequence of operations");
            }
        };
    };
};  // namespace Generator
