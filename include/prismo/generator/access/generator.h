        ...
        virtual void validate(void) const {
            if (block_size == 0) {
                throw std::invalid_argument(
                    "access_validate: block_size must be greater than zero");
            } else if (block_size > limit) {
                throw std::invalid_argument(
                    "access_validate: block_size must be less than or equal to "
                    "limit");
            }
        };
    };
    ...
    class ZipfianAccessGenerator : public AccessGenerator {
       ...
       public:
        ...
        void validate(void) const override {
            ...
            if (skew <= 0 || skew >= 1) {
                throw std::invalid_argument(
                    "zipfian_validate: skew must belong to range [0; 1]");
            }
        };
    };
};  // namespace Generator
