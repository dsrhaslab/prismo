...
namespace Parser {
    ...
    std::unique_ptr<Generator::AccessGenerator> get_access_generator(
        ...
        if (type == "sequential") {
            ...
            access_generator =
                std::make_unique<Generator::RealisticRepeatGenerator>(config);
        } else {
            throw std::invalid_argument("get_access: type '" + type +
                                        "' not recognized");
        }
    ...
    std::unique_ptr<Generator::OperationGenerator> get_operation_generator(
        ...
        if (type == "constant") {
            ...
            operation_generator =
                std::make_unique<Generator::PercentageOperationGenerator>(
                    config);
            ...
            throw std::invalid_argument("get_operation: type '" + type +
                                        "' not recognized");
        }
    ...
    std::optional<Generator::MultipleBarrier> get_multiple_barrier(
        ...
        return config.contains("barrier")
                   ? std::optional<Generator::MultipleBarrier>{config.at(
                         "barrier")}
                   : std::nullopt;
    }
    ...
    std::optional<Generator::CompressionGenerator> get_compression_generator(
        ...
        if (config.contains("compression")) {
            compression_generator =
                std::optional<Generator::CompressionGenerator>{
                    config.at("compression")};
    ...
    std::unique_ptr<Generator::ContentGenerator> get_content_generator(
        ...
        if (type == "constant") {
            ...
            content_generator =
                std::make_unique<Generator::DeduplicationContentGenerator>(
                    config);
        } else {
            throw std::invalid_argument("get_generator: type '" + type +
                                        "' not recognized");
        }
    ...
    std::unique_ptr<Logger::Logger> get_logger(const json& config) {
        ...
        if (type == "spdlog") {
            ...
            throw std::invalid_argument("get_logger: type '" + type +
                                        "' not recognized");
        }
    ...
    std::unique_ptr<Metric::Metric> get_metric(const json& config) {
        ...
        if (type == "none") {
            ...
            throw std::invalid_argument("get_metric: type '" + type +
                                        "' not recognized");
        }
    ...
    std::unique_ptr<Engine::Engine> get_engine(
        const json& config, std::unique_ptr<Metric::Metric> metric,
        std::unique_ptr<Logger::Logger> logger) {
        std::string type = config.at("type").get<std::string>();

        if (type == "posix") {
            return std::make_unique<Engine::PosixEngine>(std::move(metric),
                                                         std::move(logger));
        } else if (type == "uring") {
            return std::make_unique<Engine::UringEngine>(
                std::move(metric), std::move(logger),
            ...
            throw std::invalid_argument("get_engine: type '" + type +
                                        "' not recognized");
        }
    }
}  // namespace Parser