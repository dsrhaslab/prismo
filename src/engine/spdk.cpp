...
namespace Engine {
    ...
    SpdkEngine::SpdkEngine(std::unique_ptr<Metric::Metric> _metric,
                           std::unique_ptr<Logger::Logger> _logger,
                           const SpdkConfig& config)
        : Engine(std::move(_metric), std::move(_logger)), trigger_atomic() {
    ...
    int SpdkEngine::start_spdk_app(void* spdk_engine, const SpdkConfig config,
                                   std::atomic<TriggerData>* trigger_atomic) {
        spdk_app_opts opts = {};
        SpdkAppContext app_context = {.bdev = nullptr,
                                      .bdev_desc = nullptr,
                                      .config = config,
    ...
    void SpdkEngine::thread_main_fn(void* app_ctx) {
        ...
        int rc =
            spdk_bdev_open_ext(app_context->config.bdev_name.c_str(), true,
                               bdev_event_cb, nullptr, &app_context->bdev_desc);

        if (rc) {
            SPDK_ERRLOG("[ERROR] Could not open bdev: %s\n",
                        app_context->config.bdev_name.c_str());
            spdk_app_stop(-1);
            return;
        }
        ...
        init_thread_cb_contexts(app_context, thread_cb_contexts,
                                available_indexes, &out_standing);

        SPDK_NOTICELOG("[ALLOC] Allocating DMA buffer for I/O operations\n");
        size_t block_size = spdk_bdev_get_block_size(app_context->bdev) *
                            spdk_bdev_get_write_unit_size(app_context->bdev);

        uint8_t* dma_buf = (uint8_t*)allocate_dma_buffer(
            app_context, total_blocks, block_size);

        SPDK_NOTICELOG(
            "[DISPATCH] Starting main dispatch loop on SPDK threads\n");
        thread_main_dispatch(app_context, workers, thread_contexts,
                             thread_cb_contexts, available_indexes, dma_buf,
                             block_size);

        SPDK_NOTICELOG("[WAIT] Waiting for all callbacks to complete\n");
        while (out_standing != 0) {
            SPDK_NOTICELOG("[WAIT] Out-standing callbacks: %d\n",
        ...
        SPDK_NOTICELOG("[CLEANUP] Closing block device on current thread: %s\n",
    ...
    void SpdkEngine::init_threads(std::vector<uint32_t> pinned_cores,
    ...
    void SpdkEngine::init_thread_contexts(
        SpdkAppContext* app_context, std::atomic<bool>* submitted,
        std::vector<spdk_thread*>& workers,
        ...
        for (size_t i = 0; i < thread_contexts.size(); i++) {
            ...
            SPDK_NOTICELOG(
                "[INIT] Creating SPDK thread context for thread '%s'\n",
                thread_name);
            ...
            SPDK_NOTICELOG(
                "[SEND] Dispatching thread_setup_io_channel_cb to thread "
                "'%s'\n",
                thread_name);
            spdk_thread_send_msg(workers[i], thread_setup_io_channel_cb,
                                 thread_contexts[i]);
    ...
    void SpdkEngine::thread_setup_io_channel_cb(void* thread_ctx) {
        ...
        SpdkThreadContext* thread_context =
            static_cast<SpdkThreadContext*>(thread_ctx);

        SPDK_NOTICELOG("[SETUP] Opening I/O channel on thread '%s' (core %u)\n",
                       thread_name, spdk_env_get_current_core());
        thread_context->bdev_io_channel =
            spdk_bdev_get_io_channel(thread_context->bdev_desc);
    ...
    void* SpdkEngine::allocate_dma_buffer(SpdkAppContext* app_context,
                                          size_t total_blocks,
                                          size_t block_size) {
        ...
        SPDK_NOTICELOG(
            "[ALLOC] Allocating pinned memory buffer of size %lu, alignment "
            "%lu\n",
            buf_size, buf_align);
        uint8_t* dma_buf =
            (uint8_t*)spdk_dma_zmalloc(buf_size, buf_align, nullptr);
    ...
    void SpdkEngine::threads_cleanup(
        ...
        for (size_t i = 0; i < workers.size(); i++) {
            ...
            SPDK_NOTICELOG("[CLEANUP] Sending cleanup message to thread: %s\n",
                           thread_name);
            spdk_thread_send_msg(workers[i], thread_cleanup_cb,
                                 thread_contexts[i]);
    ...
    void SpdkEngine::thread_main_dispatch(
        SpdkAppContext* app_context, std::vector<spdk_thread*>& workers,
        std::vector<SpdkThreadContext*>& thread_contexts,
        std::vector<SpdkThreadCallBackContext*>& thread_cb_contexts,
        moodycamel::ConcurrentQueue<int>& available_indexes, uint8_t* dma_buf,
        int block_size) {
        ...
        for (int i = 0, iter = 0; true; i = (i + 1) % workers.size(), iter++) {
            ...
            TriggerData snap =
                app_context->trigger_atomic->load(std::memory_order_acquire);
            while (!snap.has_next) {
                snap = app_context->trigger_atomic->load(
            ...
            if (thread_context->request->operation ==
                Operation::OperationType::WRITE) {
                SPDK_NOTICELOG(
                    "[DISPATCH] Memcopy to DMA buffer index %d (size %zu)\n",
                    free_index, thread_context->request->size);
                std::memcpy(dma_chunk, thread_context->request->buffer,
            ...
            thread_context->thread_cb_ctx->metric_data = {
                ...
                .metadata =
                    Generator::BlockMetadata{
                        .block_id = thread_context->request->metadata.block_id,
                        .compression =
                            thread_context->request->metadata.compression},
            ...
            SPDK_NOTICELOG(
                "[DISPATCH] Waiting for submission done on thread '%s'\n",
                spdk_thread_get_name(worker));
            while (
                !thread_context->submitted->load(std::memory_order_acquire)) {
                // Optional: small pause to reduce CPU spinning
                // spdk_delay_us(1);
            }
            ...
            app_context->trigger_atomic->store(done_snap,
    ...
    void SpdkEngine::thread_fn(void* thread_ctx) {
        ...
        SPDK_NOTICELOG(
            ...
            thread_name, core, thread_context->request->size,
        ...
        if (rc == -ENOMEM) {
            ...
            spdk_bdev_queue_io_wait(thread_context->bdev,
                                    thread_context->bdev_io_channel,
                                    &thread_context->bdev_io_wait);

        } else if (rc == 0) {
            SPDK_NOTICELOG(
                "[THREAD %s | CORE %u] Submition completed successfully\n",
                thread_name, core);
    ...
    int SpdkEngine::thread_read(SpdkThreadContext* thread_ctx) {
        return spdk_bdev_read(
            thread_ctx->bdev_desc, thread_ctx->bdev_io_channel,
            thread_ctx->request->buffer, thread_ctx->request->offset,
            thread_ctx->request->size, io_complete, thread_ctx->thread_cb_ctx);
    }
    ...
    int SpdkEngine::thread_fsync(SpdkThreadContext* thread_ctx) {
        return spdk_bdev_flush(
            thread_ctx->bdev_desc, thread_ctx->bdev_io_channel,
            thread_ctx->request->offset, thread_ctx->request->size, io_complete,
    ...
    void SpdkEngine::io_complete(struct spdk_bdev_io* bdev_io, bool success,
                                 void* thread_cb_ctx) {
        SpdkThreadCallBackContext* thread_cb_context =
        ...
        if (success) {
            ...
            SPDK_ERRLOG("[THREAD %s | CORE %u] Bdev I/O error: %d\n",
                        thread_name, core, EIO);
        }

        Metric::fill_metric(*spdk_engine->metric,
        ...
        SPDK_NOTICELOG(
            ...
            thread_name, core, thread_cb_context->index);
    }
