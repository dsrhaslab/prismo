...
Com o crescimento das aplicações intesivas em @io, em particular nas áreas da inteligência artificial e análise de dados, os sistemas de armazenamento assumem um papel cada vez mais determinante no desempenho das aplicações. A capacidade de processar e armazenar grandes volumes de dados de forma eficiente tornou-se um fator crítico, tornando igualmente relevante a existência de mecanismos de avaliação, como benchmarks, que permitam analisar e comparar o comportamento destes sistemas de forma rigorosa.

Os sistemas de armazenamento modernos evoluíram no sentido de oferecer uma maior eficiência de acesso e densidade dos dados. Sistemas como o @zfs disponibilizam um conjunto alargado de funcionalidades que procuram melhorar a performance das aplicações, recorrendo a diferentes técnicas de otimização. Entre estas, destacam-se a deduplicação, que reduz o espaço de armazenamento ao evitar a reescrita de dados já existentes, e a compressão, que permite aumentar a densidade dos dados ao eliminar redundâncias e informação que pode ser reconstruída a partir de uma amostra menor.

Pórem a avaliação do impacto destas técnicas não é trivial, pois o número crescente de otimizações implementadas nos sistemas de armazenamento atuais dificulta a obtenção de métricas representativas do seu funcionamento real. Os benchmarks desempenham aqui um papel fundamental, permitindo caracterizar com rigor os sistemas e compreender de que forma determinadas decisões de implementação influenciam o comportamento observado pelas aplicações.

Entretanto, e com o objetivo de ultrapassar limitações impostas pela stack de @io tradicional do kernel, nomeadamente mudanças de contexto, interrupções e cópias entre user e kernel space (que evidenciam gargalos de desempenho), surgiram diversas @api:pl que procuram mitigar estes problemas, operando geralmente sobre runtimes assíncronos.

Por fim, a qualidade das avaliações não depende unicamente da ferramenta de benchmark, o realismo das workloads é outra propriedade essencial no levantamento de ilações corretas acerca do sistema.

=== Definição do Problema e Desafios

Os sistemas de armazenamento modernos combinam múltiplas técnicas de otimização, cujo impacto no desempenho depende fortemente das propriedades dos dados processados. A criação de workloads realistas que respeitem estas propriedades e reflitam ambientes de produção continua a ser um desafio significativo, fazendo com que as condições observadas em ambientes reais raramente sejam reproduzidas em contexto de benchmarking, o que contribui para avaliações indevidas do sistema.

Adicionalmente, a crescente diversidade de interfaces de @io expostas pelos sistemas de armazenamento dificulta a conceção de benchmarks genéricos que consigam exercitar de forma consistente e comparável os diferentes caminhos de execução, comprometendo a uniformidade da avaliação entre sistemas heterogéneos.

Para piorar a situação, muitos dos traces atualmente disponíveis à comunidade datam de vários anos e foram recolhidos em contextos tecnológicos substancialmente distintos dos atuais. Quando aplicados a sistemas modernos, estes traces são executados rapidamente, não permitindo sequer atingir um estado estável do dispositivo de armazenamento. Tornando-se por isso relevante a extensão sintética dos mesmos, de forma a preservar as propriedades observadas nos dados originais, permitindo uma avaliação mais fiel e sustentada do desempenho dos sistemas de armazenamento.

Posto isto, a seleção dos benchmarks amplamente utilizados pela comunidade revela limitações significativas e uma escassez de funcionalidades que dificultam uma avaliação rigorosa dos sistemas de armazenamento atuais. Entre essas limitações, destacam‑se as seguintes propriedades:

+ As distribuições de duplicados e compressibilidade são obtidas através de padrões simplistas que repetem constantemente o mesmo conjunto de blocos, aumentando assim a localidade temporal e espacial, o que beneficia indevidamente o sistema de armazenamento.

+ Não existe suporte direto para a interface @spdk, o que é particularmente limitativo dado esta framework ser fundamental para avaliar sistemas que operam com acesso direto ao controlador de @nvme, contornando assim a stack de @io do kernel e explorando o desempenho real.

+ As workloads descritas tendem são estáticas e pouco configuráveis, não permitindo ajustar propriedades essenciais dos dados, como a evolução da localidade espacial e temporal dos pedidos de @io, restringindo a capacidade de simular cenários realistas.

Em suma, os benchmarks atuais não permitem definir workloads realistas nem suportam múltiplas interfaces e otimizações modernas tornando impossível avaliar de forma precisa as características essenciais dos sistemas de armazenamento.

=== Objetivos e Contribuições

Perante os problemas mencionados anteriormente, esta dissertação procura melhorar a eficiência e dotar duma maior flexibilidade os algoritmos para geração de conteúdo, em particular o respeito pelas taxas de deduplicação e compressão num sistema orientado ao bloco, podendo estes ser aplicados sobre múltiplas interfaces de @io.
