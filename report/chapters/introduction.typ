...
Porém a avaliação do impacto destas técnicas não é trivial, pois o número crescente de otimizações implementadas nos sistemas de armazenamento atuais dificulta a obtenção de métricas representativas do seu funcionamento real @tarasov2011. Os benchmarks desempenham aqui um papel crucial, permitindo caracterizar com rigor os sistemas e compreender de que forma determinadas decisões de implementação influenciam o comportamento observado pelas aplicações.
...
Assim sendo, o protótipo do benchmark reúne todas as contribuições da dissertação numa arquitetura que torna as várias @api:pl de @io completamente independentes da geração de conteúdo, deste modo o utilizador final usufrui das seguintes vantagens:
...
+ Replicação completa de traces do @fiu para avaliação do sistema de armazenamento sobre cargas de trabalho reais, permitindo a identificação de gargalos em ambientes de produção e reprodutibilidade dos resultados @talwadker2014 @tracegen2024.

+ Replicação parcial de traces com geração de dados sintéticos para os parâmetros em falta, desta forma a replicação das operações de @io pode ser efetuada sobre diferentes padrões de acesso, adição de barreiras e outros critérios à escolha do utilizador @pang2026 @tracegen2024.
