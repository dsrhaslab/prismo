...
Para piorar a situação, muitos dos traces atualmente disponíveis à comunidade datam de vários anos e foram recolhidos em contextos tecnológicos substancialmente distintos dos atuais @borrill2007 @talwadker2014. Quando aplicados a sistemas modernos, estes traces são executados rapidamente, não permitindo sequer atingir um estado estável do dispositivo de armazenamento. Torna-se, por isso, relevante a extensão sintética dos mesmos, de forma a preservar as propriedades observadas nos dados originais, permitindo uma avaliação mais fiel e sustentada do desempenho dos sistemas de armazenamento @gracia-tinedo2015 @pang2026.

Posto isto, a seleção dos benchmarks amplamente utilizados pela comunidade revela limitações significativas e uma escassez de funcionalidades que dificultam uma avaliação rigorosa dos sistemas de armazenamento atuais @fio_docs @dedisbench @dedisbenchpp. Entre essas limitações, destacam-se as seguintes:

As distribuições de duplicados e compressibilidade são obtidas através de padrões simplistas que repetem constantemente o mesmo conjunto de blocos, aumentando assim a localidade temporal e espacial, o que beneficia indevidamente o sistema de armazenamento @paulo2013.

Não existe suporte direto para a interface @spdk, o que é particularmente limitativo dado esta framework ser fundamental para avaliar sistemas que operam com acesso direto ao controlador de @nvme, contornando assim a stack de @io do kernel e explorando o desempenho real @didona2022.

As workloads descritas tendem a ser estáticas e pouco configuráveis, não permitindo ajustar propriedades essenciais dos dados, como a evolução da localidade espacial e temporal dos pedidos de @io, restringindo a capacidade de simular cenários realistas @gracia-tinedo2018.

Em suma, os benchmarks atuais não permitem definir workloads realistas nem suportam múltiplas interfaces que proporcionam a exercitação das otimizações implementadas, tornando impossível avaliar de forma precisa as características essenciais dos sistemas de armazenamento.

=== Objetivos e Contribuições

Perante os problemas mencionados anteriormente, esta dissertação procura melhorar a eficiência e dotar duma maior flexibilidade os algoritmos para geração de conteúdo, em particular o respeito pelas taxas de deduplicação e compressão num sistema orientado ao bloco, podendo estes ser aplicados sobre múltiplas interfaces de @io @koller2010 @constantinescu2011.

No entanto, tal condição é insuficiente para alcançar workloads realistas que efetuem uma avaliação correta do sistema, para isto é necessário replicar as propriedades de traces extraídos a partir de ambientes em produção, pois somente estes oferecem informações acerca das cargas reais @gracia-tinedo2015 @pang2026.
...
Replicação parcial de traces com geração de dados sintéticos para os parâmetros em falta, desta forma a replicação de operações de @io pode ser efetuada sobre diferentes padrões de acesso, adição de barreiras e outros critérios à escolha do utilizador @pang2026.

Extensão de traces com dados sintéticos, assim após o conteúdo original do trace ser totalmente replicado, o mesmo é continuamente estendido pelo tempo necessário, salvaguardando as características e padrões encontrados originalmente nos dados @pang2026.

Tendo um protótipo com estas características, contribuímos para que a avaliação dos sistemas de armazenamento seja efetuada como mais critério, afinal o utilizador tem a possibilidade de testar várias interfaces de @io e para cada uma selecionar workloads que avaliem determinadas características do sistema, tudo com o maior realismo possível, e não através de métodos simplistas como praticam as demais soluções @fio_docs @dedisbench @dedisbenchpp.
