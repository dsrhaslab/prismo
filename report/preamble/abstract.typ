...
Nos dias de hoje, os sistemas em questão combinam técnicas de deduplicação e compressão para extrair maior densidade dos dados e armazenar somente o estritamente necessário. No entanto, estas otimizações tornam a avaliação mais complexa, uma vez que as workloads precisam de responder a critérios que validem deduplicação e compressão em simultâneo, sem desconsiderar características comuns entre sistemas, como propriedades de localidade espacial e temporal dos acessos.
...
Atualmente, não existe uma solução capaz de combinar todas estas características. É precisamente neste ponto que se foca este trabalho: desenvolver um benchmark para sistemas de armazenamento capaz de suportar diversas interfaces de I/O, gerar workloads realistas e recolher métricas relevantes, permitindo avaliar o desempenho do sistema, identificar gargalos e analisar o impacto das suas otimizações.
]
